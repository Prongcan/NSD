...
class RayPPOTrainer:
    ...
    def _validate(self, merged: bool = False):
        ...
        for test_data in self.val_dataloader:
            ...
            # Compute response length stats for truncation rate
            if "response_length" in test_batch.batch:
                val_response_lengths.extend(test_batch.batch["response_length"].float().cpu().tolist())
            else:
                response_mask = test_batch.batch.get("response_mask", None)
                if response_mask is not None:
                    val_response_lengths.extend(response_mask.sum(-1).float().cpu().tolist())
            if val_max_response_length is None:
                val_max_response_length = test_batch.batch["responses"].shape[-1]
        ...
        return self._val_metrics_update(
            ...
            val_max_response_length=val_max_response_length,
        )

    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns,
                            val_response_lengths=None, val_max_response_length=None):
        ...
        if val_response_lengths and val_max_response_length is not None:
            resp_lens = np.array(val_response_lengths)
            metric_dict["val-aux/response_length/mean"] = float(resp_lens.mean())
            metric_dict["val-aux/response_length/max"] = float(resp_lens.max())
            metric_dict["val-aux/response_length/min"] = float(resp_lens.min())
            clip_ratio = float((resp_lens >= val_max_response_length).mean())
            metric_dict["val-aux/response_length/clip_ratio"] = clip_ratio
    ...
    def _compute_canon_consensus(self, batch: DataProto) -> tuple["DataProto", dict]:
        """Compute CANON consensus from N rollouts and get teacher logprobs.

        After rollout generation, decode responses, find majority-vote consensus,
        build consensus-conditioned teacher prompts, and compute teacher logprobs.
        """
        from verl.trainer.ppo.canon import batch_extract_consensus, build_canon_teacher_prompt

        n = self.config.actor_rollout_ref.rollout.n
        bsz = len(batch)
        num_prompts = bsz // n

        # Cache tokenizer for decoding
        if not hasattr(self, "_canon_tokenizer"):
            from transformers import AutoTokenizer
            model_path = self.config.actor_rollout_ref.model.path
            self._canon_tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
        tokenizer = self._canon_tokenizer
        ...
        for i in range(bsz):
            ...
            valid_ids = response_ids[:valid_len]
            text = tokenizer.decode(valid_ids.tolist(), skip_special_tokens=True)
            responses_text.append(text)

        # Get log probs for solution selection (from rollout)
        log_probs = None
        if "rollout_log_probs" in batch.batch:
            response_mask = batch.batch["response_mask"]
            rollout_lps = batch.batch["rollout_log_probs"]
            log_probs = (rollout_lps * response_mask).sum(dim=-1) / response_mask.sum(dim=-1).clamp(min=1)

        # Extract consensus
        selection = self.distillation_config.canon_solution_selection
        consensus_answers, best_solutions, consensus_ratios = batch_extract_consensus(
            responses_text, n, log_probs, selection
        )

        # Build teacher prompts for each sample
        # Use "raw_prompt" (agent_loop format) or "prompt" depending on what's available
        teacher_prompts = []
        prompt_key = "raw_prompt" if "raw_prompt" in batch.non_tensor_batch else "prompt"
        for i in range(num_prompts):
            prompt_content = batch.non_tensor_batch[prompt_key][i * n]
            if hasattr(prompt_content, "item"):
                prompt_content = prompt_content.item()
            ...
            if isinstance(prompt_content, list) and len(prompt_content) > 0:
                problem_text = prompt_content[0].get("content", "")
            else:
                problem_text = str(prompt_content)

            if best_solutions[i] is not None:
                teacher_prompt = build_canon_teacher_prompt(problem_text, best_solutions[i])
            else:
                # No consensus: teacher prompt = student prompt (unconditional)
                if isinstance(prompt_content, list):
                    teacher_prompt = prompt_content
                else:
                    teacher_prompt = [{"role": "user", "content": problem_text}]

            for _ in range(n):
                teacher_prompts.append(teacher_prompt)

        # Compute teacher logprobs with consensus-conditioned prompts via AgentLoopManager
        teacher_logprobs, teacher_ids = self.async_rollout_manager.compute_canon_teacher_logprobs(batch, teacher_prompts)
        batch.batch["teacher_logprobs"] = teacher_logprobs
        batch.batch["teacher_ids"] = teacher_ids

        # Metrics
        avg_ratio = sum(consensus_ratios) / len(consensus_ratios) if consensus_ratios else 0.0
        has_consensus_ratio = sum(1 for s in best_solutions if s is not None) / len(best_solutions) if best_solutions else 0.0

        # Log teacher prompt token lengths to detect any context overflow risk
        teacher_prompt_lens = []
        for tp in teacher_prompts[::n]:  # one per prompt group
            ids = tokenizer.apply_chat_template(tp, add_generation_prompt=True, tokenize=True)
            teacher_prompt_lens.append(len(ids))
        tp_len_max = max(teacher_prompt_lens) if teacher_prompt_lens else 0
        tp_len_mean = sum(teacher_prompt_lens) / len(teacher_prompt_lens) if teacher_prompt_lens else 0
        # Max total = teacher_prompt_len + max_response_len (worst case teacher sees longest student response)
        max_response_len = self.config.data.max_response_length
        tp_max_total = tp_len_max + max_response_len

        canon_metrics = {
            "canon/consensus_ratio": avg_ratio,
            "canon/has_consensus_frac": has_consensus_ratio,
            "canon/teacher_prompt_len_mean": tp_len_mean,
            "canon/teacher_prompt_len_max": tp_len_max,
            "canon/teacher_seq_max_total": tp_max_total,
        }

        return batch, canon_metrics
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # CANON: compute consensus and teacher logprobs post-rollout
                    canon_mode = getattr(self.distillation_config, "canon_mode", False) if self.distillation_config else False
                    if canon_mode:
                        with marked_timer("canon_consensus", timing_raw, color="green"):
                            batch, canon_metrics = self._compute_canon_consensus(batch)
                            metrics.update(canon_metrics)
                ...
                # Online NSD stats (panel: "Attack prompt generation solution")
                _online_nsd_keys = {
                    "online_nsd_rollout_len": "Attack prompt generation solution/rollout_len",
                    "online_nsd_attack_instr_len": "Attack prompt generation solution/attack_instr_len",
                    "online_nsd_attack_total_len": "Attack prompt generation solution/attack_total_len",
                    "online_nsd_attack_success": "Attack prompt generation solution/success_rate",
                }
                for src_key, dst_key in _online_nsd_keys.items():
                    if src_key in batch.non_tensor_batch:
                        vals = batch.non_tensor_batch[src_key]
                        if vals is not None and len(vals) > 0:
                            arr = np.asarray([v for v in vals if v is not None], dtype=np.float32)
                            if len(arr) > 0:
                                metrics[f"{dst_key}/mean"] = float(np.mean(arr))
                                metrics[f"{dst_key}/max"] = float(np.max(arr))
                                if src_key != "online_nsd_attack_success":
                                    metrics[f"{dst_key}/min"] = float(np.min(arr))
