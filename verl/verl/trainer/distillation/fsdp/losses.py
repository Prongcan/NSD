...
def kl_divergence(log_q: torch.Tensor, log_p: torch.Tensor, pointwise_clamp: float | None = None) -> torch.Tensor:
    """Compute KL divergence between two distributions given their log probabilities."""
    log_p = log_p.float()
    log_q = log_q.float()
    p = log_p.exp()
    kld = p * (log_p - log_q)
    if pointwise_clamp is not None:
        kld = kld.clamp(max=pointwise_clamp)
...
def compute_forward_kl_topk(
    ...
    distillation_losses = kl_divergence(
        log_q=student_topk_log_probs, log_p=teacher_topk_log_probs, pointwise_clamp=loss_config.pointwise_clamp
    )
