...
def left_right_2_no_padding(data: TensorDict) -> TensorDict:
    ...
    # (bsz, seqlen, topk) - attack teacher log-probs for negative self-distillation
    attack_teacher_logprobs = data.get("attack_teacher_logprobs", None)
    if attack_teacher_logprobs is not None:
        attack_teacher_logprobs_rmpad = index_first_axis(
            attack_teacher_logprobs.unsqueeze(-1).flatten(0, 1), indices
        )
        attack_teacher_logprobs_nested = torch.nested.nested_tensor_from_jagged(
            attack_teacher_logprobs_rmpad.squeeze(-1), offsets=cu_seqlens
        )
        data["attack_teacher_logprobs"] = attack_teacher_logprobs_nested
