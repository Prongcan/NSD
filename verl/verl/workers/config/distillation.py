...
class DistillationLossConfig(BaseConfig):
    ...
    # Per-token pointwise clamp: clamp each vocab entry's KL contribution before summing.
    # Used by forward_kl_topk to prevent stylistic tokens from dominating (OPSD paper).
    pointwise_clamp: Optional[float] = None

    # Supervised distillation clip: when use_policy_gradient=False, optionally apply a
    # trust-region mask using the rollout ratio π_θ(a_t)/π_θ_old(a_t).
    # Tokens whose ratio falls outside [1-supervised_clip_ratio, 1+supervised_clip_ratio]
    # are excluded from the loss, preventing runaway entropy growth.
    # Set to None to disable (default), or e.g. 0.2 to enable.
    supervised_clip_ratio: Optional[float] = None

    # Divergence-gated unlikelihood loss (divergence_gated mode only)
    alpha: float = 1.0  # Weight for the push (unlikelihood) term
    kl_coeff: float = 0.01  # Weight for KL penalty against reference model (prevent drift)

    # KL estimator to use when topk pre-computed loss is unavailable.
    # "k1": log π_c - log π_ref  (sampled-token, biased, no restoring force in supervised mode)
    # "forward": Σ_{v in topk} π_ref(v) * (log π_ref(v) - log π_c(v))  (forward KL over topk vocab)
    kl_mode: str = "k1"
...
class DistillationConfig(BaseConfig):
    ...
    enabled: bool = False
    online_nsd: bool = False
    canon_mode: bool = False
    canon_consensus_threshold: float = 0.0
    canon_solution_selection: str = "highest_logprob"
