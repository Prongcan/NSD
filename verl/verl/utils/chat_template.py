...
def initialize_system_prompt(tokenizer, **apply_chat_template_kwargs) -> list[int]:
    ...
    try:
        # Some chat templates (e.g., Mistral) require alternating user/assistant roles
        # and will reject consecutive user messages. Fall back to user/assistant pair.
        ...
        # If consecutive user messages fail, try with user/assistant pair
        token2 = normalize_token_ids(
            tokenizer.apply_chat_template(
                [
                    {"role": "user", "content": ""},
    ...
    # get system prompt tokens (difference between first and second message)
