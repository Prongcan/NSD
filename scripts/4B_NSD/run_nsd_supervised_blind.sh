...
# Full MATH dataset | Blind adversarial attack prompt (4B-instruct-generated, problem-only)
...
# Attack prompts: pre-generated blind adversarial context (math_nsd_every_4b_instruct)
#
# KEY DIFFERENCE from run_nsd_supervised_sol_aware.sh:
#   Attack data: math_nsd_every_4b_instruct  → blind attacks (problem only, no solution hint)
#                vs math_nsd_every_4b_sol_aware (solution-aware attacks)
...
# Blind attack prompts are short (problem only); 8192 is sufficient
...
train_data=${DATA_DIR}/math_nsd_every_4b_instruct/train.parquet
test_data=${DATA_DIR}/math_nsd_every_4b_instruct/test_200.parquet
...
########################### launch ###########################
LOG_DIR="${SCRIPT_DIR}/logs_supervised_blind_fwdkl"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/nsd_supervised_train_$(date +%Y%m%d_%H%M%S).log"

echo "=== NSD Supervised Distillation (blind, use_policy_gradient=False) ==="
