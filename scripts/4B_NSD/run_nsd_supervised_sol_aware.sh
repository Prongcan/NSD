...
# Full MATH dataset | Solution-aware attack prompt (4B-instruct-generated)
...
# Attack prompts: pre-generated solution-aware adversarial context (math_nsd_every_4b_sol_aware)
#
# KEY DIFFERENCE from run_negative_distill_every_log_gate_full_instruct.sh:
...
# Sol-aware prompts include problem + solution context; 8192 sufficient
...
experiment_name=qwen3_4b_nsd_supervised_sol_aware_fwdkl
default_local_dir=${CHECKPOINT_ROOT}/${project_name}/${experiment_name}
# ---- end user-adjustable ----

DATA_DIR=${PROJECT_ROOT}/data
train_data=${DATA_DIR}/math_nsd_every_4b_sol_aware/train.parquet
test_data=${DATA_DIR}/math_nsd_every_4b_sol_aware/test_200.parquet
...
########################### launch ###########################
LOG_DIR="${SCRIPT_DIR}/logs_supervised_sol_aware_fwdkl"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/nsd_supervised_train_$(date +%Y%m%d_%H%M%S).log"

echo "=== NSD Supervised Distillation (sol-aware, use_policy_gradient=False) ==="
