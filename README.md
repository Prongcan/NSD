...
**Attack variants supported:**
- *Question-only (blind)* — LLM-generated attack prompt, without seeing the solution
- *Sol-aware* — LLM-generated attack prompt, conditioned on the solution
- *Wiki-irr* — inject a random Wikipedia article as irrelevant context ([source](https://arxiv.org/abs/2504.02111))
- *Sol-aware online* — attack prompts generated on-the-fly each training step
...
```
negative-sd/
├── prompt/                              # Attack prompt generation pipelines
│   ├── main_every.py                    # Blind attack (per-problem, vLLM server)
│   ├── main_every_4b_sol_aware.py       # Sol-aware attack (4B generator)
│   ├── main_wiki_irrelevant.py          # Wikipedia irrelevant context injection
│   └── attack_prompt/                   # Raw attack prompt templates
├── scripts/
...
│   ├── math_nsd_every_4b_sol_aware/     # Sol-aware attack dataset (4B-generated) *
│   ├── math_online_nsd/                 # Online NSD dataset (4B sol-aware, for online training) *
│   ├── math_nsd_wiki_irrelevant/        # Wiki-irr attack dataset
...
> **\* Data note:** `math_nsd_every_4b_sol_aware` and `math_online_nsd` are the example training datasets used by the `4B_NSD` scripts — attack prompts generated by Qwen3-4B.
>
