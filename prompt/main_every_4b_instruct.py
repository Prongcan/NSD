"""
Per-sample NSD prompt generation pipeline.

For EACH training sample, generate a specific attack prompt tailored to that problem.
Uses Qwen3-4B (base, instruct mode via chat template, no-example prompt).
"""
...
CACHE_DIR = Path(__file__).parent / "cache_every_4b_instruct"
...
class Config:
    ...
    output_dir: str = "data/math_nsd_every_4b_instruct"
    seed: int = 42
    max_workers: int = 30
    resume: bool = False
...
_PROMPT_FILE = Path(__file__).parent / "attack_prompt" / "no_example.txt"
ERROR_GENERATION_PROMPT = _PROMPT_FILE.read_text()
...
def generate_attack_prompt(index: int, problem: str, config: Config) -> Optional[tuple[int, str]]:
    ...
    try:
        prompt = ERROR_GENERATION_PROMPT.format(problem=problem)
        ...
        attack_prompt = clean_attack_prompt(raw)

        if attack_prompt:
            cache_file.write_text(attack_prompt)
            return (index, attack_prompt)
        else:
            print(f"  [Warning] Failed to clean output for index {index}")
            return None
...
def generate_train_dataset(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    print(f"  Generating attack prompts for {len(df)} training samples...")
    ...
    with ThreadPoolExecutor(max_workers=config.max_workers) as executor:
        futures = {
            executor.submit(generate_attack_prompt, i, row["problem"], config): i
...
def main():
    parser = argparse.ArgumentParser(description="Generate per-sample NSD dataset with Qwen3-4B (no-example prompt)")
    ...
    parser.add_argument("--output_dir", default="data/math_nsd_every_4b_instruct")
    ...
    print("=" * 60)
    print("Per-Sample NSD Dataset Generation (Qwen3-4B, no-example)")
    print("=" * 60)
    ...
    print(f"  {sample['attack_teacher_prompt'][0]['content'][:200]}...")
    print("\nDone!")
